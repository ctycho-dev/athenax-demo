 ...
 * No API key required!
 */
export const fetchYouTubeData = async (videoUrl: string): Promise<YouTubeOEmbedData | null> => {
   try {
      // Check cache first
 ...
 */
export const getYouTubeThumbnail = async (videoUrl: string): Promise<string> => {
   const data = await fetchYouTubeData(videoUrl);
