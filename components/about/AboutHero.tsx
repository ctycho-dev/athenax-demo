...
export const Pillars = ({ items }: PillarsProps) => (
   <div className="grid grid-cols-2 gap-8">
      {items.map((item, index) => {
         ...
         return (
            <div key={index} className={`p-8 ${bgClass} flex flex-col justify-center space-y-4`}>
               <h5 className="font-medium text-xl">{item.title}</h5>
               <p className={`text-[10px] font-bold uppercase tracking-[0.2em] ${textClass}`}>
                  {item.description}
               </p>
            </div>
         );
      ...
      <div className="max-w-7xl mx-auto">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-24">
            <div className="space-y-8 text-lg font-light leading-relaxed text-[#0a0a14]/80">
               <p>
                  AthenaX is a decentralized incubator designed to replace venture burn with
                  yield-funded growth.
               </p>
               <p>
                  We help builders secure non-dilutive funding , then amplify capital through vault
                  yields and media distribution.
               </p>
