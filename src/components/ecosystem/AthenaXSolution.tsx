export const AthenaXSolution = () => (
    <section className="py-24 px-6 bg-[#F5F5F7]">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20">
            <div>
                <h2 className="text-4xl font-bold mb-10">The AthenaX Solution</h2>
                <p className="text-2xl leading-relaxed text-gray-600 mb-8">
                    AthenaX operates as your embedded incubation partner. Full-stack infrastructure that aligns capital, narrative, and distribution.
                </p>
            </div>
            <div className="flex items-center">
                <div className="p-8 bg-white rounded-xl border border-gray-200">
                    <h4 className="font-bold mb-4 uppercase tracking-widest text-xs text-blue-600">The Outcome</h4>
                    <p className="text-lg font-bold">Sustainable, yield-funded growth. Principal untouched. Builder activity compounding.</p>
                </div>
            </div>
        </div>
    </section>
);
