...
export const Navigation = () => {
    ...
    const isDarkPage = location.pathname.includes('/tv') || location.pathname.includes('/ecosystem');
    ...
    const logoAccent = isDarkPage ? 'text-blue-400' : 'text-[#4a6fa5]';
    const ctaBorder = isDarkPage ? 'border-white' : 'border-[#0a0a14]';
    const ctaHover = isDarkPage ? 'hover:text-blue-400 hover:border-blue-400' : 'hover:text-[#4a6fa5] hover:border-[#4a6fa5]';

    return (
        <nav className={`fixed top-0 w-full z-50 transition-all duration-500 border-b ${scrolled ? `${bgScrolled} backdrop-blur-md py-4 ${borderColor}` : 'bg-transparent py-6 border-transparent'} ${textColor}`}>
            <div className="max-w-7xl mx-auto px-8 flex justify-between items-center">
                <div className="flex items-center gap-12">
                    <Link to="/v3" className="text-xl font-bold tracking-tighter">
                        ATHENA<span className={logoAccent}>X</span>
                    </Link>
